...
import DonorList from '../../components/Donation/DonorList.js';
import type { DonationData } from '../../types/Donation.js';
...
describe('DonorList component', () => {
    ...
    it('shows the same relative time whether Z is present or missing', () => {
        const withoutZ = makeDonation({ donation_id: 'a', created_at: '2026-01-01T09:55:00.000' });
        const withZ = makeDonation({ donation_id: 'b', created_at: '2026-01-01T09:55:00.000Z' });

        const { container: containerA } = render(React.createElement(DonorList, { donations: [withoutZ] }));
        const { container: containerB } = render(React.createElement(DonorList, { donations: [withZ] }));
