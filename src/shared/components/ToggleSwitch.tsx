...
const Switch = ({ isOn, onToggle, disabled }: SwitchProps) => (
    <button
        ...
        className={`flex h-5 w-10 items-center p-0.5 transition-all duration-300 ${
            isOn ? 'bg-gradient-to-br from-blue-500 to-blue-600' : 'bg-gray-300 dark:bg-zinc-700'
...
const ToggleSwitch = ({
        ...
        return (
            ...
            >
                ...
                <div className="shrink-0">
                    <Switch isOn={isOn} onToggle={onToggle} disabled={disabled ?? false} />
                </div>
            </motion.div>
        );
    }
    ...
    return (
        <div className="flex items-center justify-between py-1.5">
            {label && <span className="text-sm text-gray-800 dark:text-gray-100">{label}</span>}
